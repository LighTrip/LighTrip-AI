...
검토 대상은 `data_places365_2`와 `configs/places365_categories_v2.json`입니다. 기존 예시 label 중 v2에서 제외된 `cafeteria`, `bookstore` 등은 `data_places365`에 남아 있는 샘플을 함께 확인했습니다.
...
- `기타` 카테고리는 `data_places365_2`에 없습니다. 서비스 분류기가 `기타`를 필요로 한다면 별도 negative/unknown 데이터셋으로 보강해야 합니다.
